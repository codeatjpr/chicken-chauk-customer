...
export function SignupFlow({ onSuccess, onCancel, from = ROUTES.home }: SignupFlowProps) {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      toast.success(data.isNewUser ? 'Account created' : 'Signed in')

      const trimmed = pendingName.trim()
      let finalUser = data.user
      if (trimmed.length >= 2) {
        try {
          finalUser = await profileApi.updateProfile({ name: trimmed })
          setUser(finalUser)
        } catch (e) {
          console.error(e)
          toast.error('Could not save your name. You can add it in profile.')
        }
      }

      if (profileNeedsDisplayName(finalUser)) {
    ...
    return (
      <div className="space-y-4">
        <form onSubmit={onDetailsSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="sf-phone">Mobile number</Label>
