...
function useOrderShortId(id: string) {
  const qc = useQueryClient()
  const cached = qc.getQueryData<{ shortId?: string; id?: string }>(
...
function VendorCrumbs({ id }: { id: string }) {
  ...
  const crumbs: Crumb[] = [{ label: name ?? 'Vendor', to: `/vendors/${id}` }]
...
function ProductCrumbs({ id }: { id: string }) {
  ...
  const crumbs: Crumb[] = [
    { label: 'Products', to: ROUTES.browse },
    { label: name ?? 'Product' },
  ]
  return <CrumbList crumbs={crumbs} />
}
...
export function AppBreadcrumb() {
  ...
  return (
    <nav
      ...
      className="mb-4 hidden items-center gap-1 lg:flex"
    >
      ...
      {/* Static routes */}
      {!vendorMatch && !productMatch && !categoryMatch && !orderMatch && !trackingMatch && (
